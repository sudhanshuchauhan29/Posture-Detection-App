...
def print_all_metrics_from_saved_results(results_store, le):
    for split_id in sorted(results_store.keys()):
        ...
        if "y_true" in cnn_data:
            print_metrics(
                np.array(cnn_data["y_true"]),
                np.array(cnn_data["y_pred"]),
                "CNN",
                split_id,
                le.classes_
            )
        ...
        if "y_true" in hybrid_data:
            print_metrics(
                np.array(hybrid_data["y_true"]),
                np.array(hybrid_data["y_pred"]),
...
def train_all_models(splits, le):
    ...
    for split_id in range(4):
        ...
        for name, clf in ml_models.items():
            ...
            print_metrics(y_test, preds, name, split_id+1, le.classes_)
            ...
            joblib.dump(clf, f"ml_model_split{split_id+1}_{name}.pkl")
        ...
        print_metrics(y_test, y_pred_cnn, "CNN", split_id+1, le.classes_)
        ...
        cnn.save(f"cnn_split{split_id+1}.h5")
        ...
        print_metrics(y_test, y_pred_hybrid,
                      "Hybrid Conv1D–BiLSTM",
                      split_id+1,
                      le.classes_)
        ...
        hybrid.save(f"hybrid_split{split_id+1}.h5")
...
if __name__ == "__main__":
    ...
    if os.path.exists("results_store.json"):
        print("✔ Saved results found. Loading...")
        with open("results_store.json", "r") as f:
